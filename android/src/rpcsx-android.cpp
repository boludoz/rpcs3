...
// NOTE: the rpcsx/rpcs3 fork this file was ported from added
// cfg::_base::to_json()/from_json() (nlohmann::json-based) so the Android
// settings screen can read/write individual config nodes. Vanilla rpcs3's
// cfg::_base (Utilities/Config.h) only has the plain to_string()/from_string()
// pair used by rpcs3's own regular settings load/save - it does not have
// to_json()/from_json(). Rather than guess at a JSON schema the Kotlin side
// may or may not already depend on, these two JNI entry points are stubbed
// out for now so the rest of the emulator core still compiles and runs.
