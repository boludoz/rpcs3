...
extern "C" int _rpcsx_bootIsoFd(int fd) {
  if (fd < 0) {
  ...
  int result = static_cast<int>(Emu.BootGame(
      iso_device::virtual_device_name + "/", "", false, cfg_mode::custom));
  rpcsx_android.error("_rpcsx_bootIsoFd: BootGame returned %d (fd=%d)", result, fd);
...
// --- Settings bridge --------------------------------------------------------
// One system for global and per-title configuration, desktop-style:
//  - The YAML files the core itself reads are the single source of truth:
//    config.yml (global) and custom_configs/config_<TITLE_ID>.yml (per-title,
//    applied by the core at boot via cfg_mode::custom).
//  - Writes edit the YAML document and save it through Emulator::SaveSettings.
//    The emulator does NOT need to be stopped: global writes are additionally
...
//    "min"/"max" for integers), with the file values overlaid and per-title
//    overrides flagged as "overridden".
...
// Serializes `node` (and children) as the UI schema. `global_overlay` and
// `title_overlay` carry the matching subtrees of config.yml and of the
// title's custom config (Null where absent); scalar values found there win
// over the in-memory tree, so the UI always shows what is actually saved.
static void cfg_to_json(const cfg::_base *node,
                        const YAML::Node &global_overlay,
                        const YAML::Node &title_overlay, bool title_scope,
                        std::string &out) {
  const auto effective_value = [&]() -> std::string {
    if (title_scope && title_overlay.IsScalar()) {
      return title_overlay.Scalar();
    }
    if (global_overlay.IsScalar()) {
      return global_overlay.Scalar();
    }
    return node->to_string();
  };

  const auto append_overridden = [&] {
    if (title_scope) {
      // Per-title files are full copies of the global config (aps3e-style),
      // so "overridden" means "differs from the global value", not merely
      // "present in the file".
      const std::string base =
          global_overlay.IsScalar() ? global_overlay.Scalar() : node->to_string();
      out += ",\"overridden\":";
      out += (title_overlay.IsScalar() && title_overlay.Scalar() != base)
                 ? "true"
                 : "false";
    }
  };
  ...
  case cfg::type::node: {
    ...
    for (const auto &child : static_cast<const cfg::node *>(node)->get_nodes()) {
      ...
      out += ':';
      cfg_to_json(child, yaml_child(global_overlay, child->get_name()),
                  yaml_child(title_overlay, child->get_name()), title_scope,
                  out);
    }
    out += '}';
    break;
...
// The YAML file backing a scope: global config.yml, or the per-title custom
// config the core applies at boot.
static std::string settings_file_path(std::string_view titleId) {
  return titleId.empty()
             ? fs::get_config_dir(true) + "config.yml"
             : rpcs3::utils::get_custom_config_path(std::string(titleId));
}

static YAML::Node settings_file_load(std::string_view titleId) {
  std::string text;
  if (fs::file f{settings_file_path(titleId)}) {
    text = f.to_string();
  }
...
static void settings_file_save(std::string_view titleId,
                               const YAML::Node &root) {
  // An empty per-title config means "no overrides": drop the file so the
  // core skips it at boot. The global config.yml is never deleted.
  if (!titleId.empty() && (!root.IsMap() || root.size() == 0)) {
    fs::remove_file(settings_file_path(titleId));
    return;
  }

  YAML::Emitter out;
  out << root;
  Emulator::SaveSettings(std::string_view(out.c_str(), out.size()),
                         std::string(titleId));
}

// Schema for the settings UI. Empty titleId = global scope; otherwise the
// title's overrides are merged in and flagged per leaf as "overridden".
extern "C" std::string _rpcsx_configGet(std::string_view titleId,
  ...
  const bool title_scope = !titleId.empty();
  YAML::Node global_overlay = settings_file_load({});
  YAML::Node title_overlay = title_scope
                                 ? settings_file_load(titleId)
                                 : YAML::Node(YAML::NodeType::Null);

  for (const auto &seg : fmt::split(path, {"@@"})) {
    global_overlay = yaml_child(global_overlay, seg);
    title_overlay = yaml_child(title_overlay, seg);
  }

  std::string result;
  cfg_to_json(node, global_overlay, title_overlay, title_scope, result);
  return result;
}
...
extern "C" bool _rpcsx_configSet(std::string_view titleId,
  ...
  // Pure per-title config (aps3e-style): the first edit seeds the file as a
  // full copy of the current global config, then edits apply on top. The
  // core overlays it over the global at boot, so a full copy behaves
  // identically while keeping the file self-contained and inspectable.
  YAML::Node root;
  if (!titleId.empty() && !fs::is_file(settings_file_path(titleId))) {
    root = settings_file_load({});
  } else {
    root = settings_file_load(titleId);
  }
  YAML::Node cur = root;
  ...
  settings_file_save(titleId, root);

  if (titleId.empty()) {
    // Best-effort live apply: dynamic settings take effect immediately, the
    // rest are already saved and apply on next boot.
...
// Removes one per-title override (empty path = reset the whole title config).
// For the global scope, removing a node reverts it to the built-in default at
// next load.
extern "C" bool _rpcsx_configRemove(std::string_view titleId,
  ...
  YAML::Node root = settings_file_load(titleId);
