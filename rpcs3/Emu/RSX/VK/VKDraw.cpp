	...
	for (u32 textures_ref = current_fp_metadata.referenced_textures_mask, i = 0; textures_ref; textures_ref >>= 1, ++i)
	{
		...
		*sampler_state = m_texture_cache.upload_texture(*m_current_command_buffer, tex, m_rtts);
		...
		if (min_filter.sample_mipmaps && mipmap_count > 1)
		{
			...
			{
				// Fallback: use the GCM mipmap count from the texture registers.
				// Some games (e.g. PES 2010) upload textures via blit_engine_dst with a
				// full mipmap chain, so forcing actual_mipmaps=1 here clamps max_lod to 0
				// and causes distant objects to sample an out-of-range LOD → black textures.
				actual_mipmaps = static_cast<f32>(mipmap_count);
			}
